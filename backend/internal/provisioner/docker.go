...
// permissioned Unix socket. Route management is delegated to CaddyRouter —
// the deep module for Caddy route ownership, ETag concurrency, and hostname
// normalization (ADR 0008: Docker is the runtime, Caddy is the routing backend).
type Docker struct {
	engine           *http.Client
	caddy            *CaddyRouter
	allowedImage     string
...
func NewDocker(socketPath, caddyAPIURL, caddyServerID, allowedImage string) (*Docker, error) {
	...
	router, err := NewCaddyRouter(caddyAPIURL, caddyServerID)
	if err != nil {
		return nil, err
	}
...
func (d *Docker) CreateSite(ctx context.Context, spec SiteSpec) error {
	...
	if err := d.caddy.UpsertRoute(ctx, spec.SiteID, []string{spec.Domain}, hostPort, true); err != nil {
...
func (d *Docker) DeleteSite(ctx context.Context, ref SiteRef) error {
	...
	if err := d.caddy.DeleteRoute(ctx, ref.SiteID, ""); err != nil {
		return err
	}
	if found {
...
func (d *Docker) ResumeSite(ctx context.Context, ref SiteRef) error {
	...
	return d.caddy.UpsertRoute(ctx, ref.SiteID, []string{domain}, port, true)
}
...
func (d *Docker) SetSiteDomains(ctx context.Context, ref SiteRef, hostnames []string) error {
	...
	if len(hostnames) == 0 {
		return d.caddy.DeleteRoute(ctx, ref.SiteID, "")
	}
	...
	return d.caddy.UpsertRoute(ctx, ref.SiteID, hostnames, hostPort, false)
}
