...
export function ProjectLogsViewer({ projectId, services }: ProjectLogsViewerProps) {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <div role="log" aria-live={live ? 'polite' : 'off'} aria-label="Project logs" className="max-h-[520px] min-h-72 overflow-auto rounded-md border bg-background p-4 font-mono text-xs leading-5 text-foreground">
          {error ? <p className="text-destructive">{error}</p> : null}
          {!error && !loading && entries.length === 0 ? <p className="text-muted-foreground">No logs match these filters yet.</p> : null}
